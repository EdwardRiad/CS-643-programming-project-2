...
# Read trained model
model = RandomForestModel.load(sc, "s3://progproj2-s3/cs643-trainingmodel.model/")
print(model)

# Reading TestValidation.csv
df = sqlContext.read.format('com.databricks.spark.csv').options(header='true', inferschema='true', sep=';').load('s3://progproj2-s3/ValidationDataset.csv')
